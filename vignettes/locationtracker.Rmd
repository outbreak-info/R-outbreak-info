...
who_prevalence = getPrevalence(pangolin_lineage = c(alpha_lineages, epsilon_lineages, iota_lineages, delta_lineages), location = "United States")
...
```{r variant_overlay}
plotPrevalenceByLocation(who_prevalence, labelDictionary = who_labels)
```

### More advanced options: specifying variants that aren't lineages
It's easy to gather the prevalence over time for a specific Pango lineage, or a group of Pango lineages, like the ones that make up WHO Variants of Concern and Interest. But you can also look at how a particular **mutation** is increasing or decreasing in prevalence or how a **lineage with a specific mutation(s)** is changing. Here, we'll look at the changes in a particular mutation within Delta (S:A222V) and compare it's growth with the mutation S:E484K, which is often found in lineages like B.1.351/Beta and P.1/Gamma, and the combination of S:E484K and S:P681H, which is found in P.1.7.
```{r custom_variants}
delta_a222v = getPrevalence(pangolin_lineage = delta_lineages, mutations="S:A222V", location = "United States")
e484k = getPrevalence(mutations="S:E484K", location = "United States")
e484k_p618h = getPrevalence(mutations=c("S:E484K", "S:P681H"), location = "United States")

df = bind_rows(delta_a222v, e484k, e484k_p618h)

# Come up with nicer labels than all the AY.1 lineages exist today
custom_labels = c("Delta with A222V", "E484K", "E484K and P681H")
names(custom_labels) = df %>% count(lineage) %>% pull(lineage)
plotPrevalenceByLocation(df, title = "Variant prevalence in the United States", labelDictionary = custom_labels)
```
...
maxDate = delta_by_state %>% pull(date) %>% max()

plotPrevalenceByLocation(delta_by_state, colorVar = "location", title = "Delta prevalence across locations") +
  xlim(c(as.Date("2021-04-01"), maxDate))
