...
resources_by_source$type = factor(resources_by_source$type, resources_by_type %>% pull(term))
resources_by_source$term = factor(resources_by_source$term, resources_by_curator %>% rev())
...
ggplot(resources_by_source %>% filter(type %in% c("Publication", "ClinicalTrial", "Dataset", "Protocol")),
       aes(x = type, y = count, fill = term, colour = term, label = term)) +
  geom_col(position = position_dodge2(width = 0.95, preserve = "single"), width = 1) +
  geom_text(aes(y = count + 1000), position = position_dodge2(width = 0.95, preserve = "single"), hjust = 0, vjust = 0.5) +
  coord_flip() +
  scale_y_continuous(labels=scales::comma, expand = c(0, 10000)) +
  scale_colour_manual(values = COLORPALETTE) +
  scale_fill_manual(values = COLORPALETTE) +
  ggtitle("Resources in outbreak.info by resource type and source") +
  theme_minimal() +
  theme(legend.position = "none", panel.grid.major.y = element_blank(), axis.title = element_blank(), text = element_text(size = 18))
```
...
### General queries
This will search for any use of the term within all the metadata, including its name, description, authors, etc. You can also combine terms using `AND` and/or `OR`. Note that spaces, dashes, and other punctuation are automatically treated as `OR` combinations.  Adding quotes will search for that exact term. **Note that all searches are case insensitive, so looking for "Pfizer" or "pfizer" will give you the same results.**
